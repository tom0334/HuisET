...
class FragmentMain : HuisEtFragment(), TurfRecAdapter.TurfHandler {
    ...
    private val showConfettiOnTurf by lazy {
        PreferenceManager.getDefaultSharedPreferences(this.context)
            .getBoolean(PREFS_TURF_CONFETTI_ID, true)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fab.setOnClickListener {
            val turfRecAdapter =
                view.findViewById<RecyclerView>(R.id.mainPersonRec).adapter as TurfRecAdapter
            ...
            builder.setMessage("Er worden momenteel geen gebruikers getoond.")
                .setPositiveButton("Naar \"Gebruikers\"") { _, _ ->
                    ...
                    val bottomNavigation =
                        context.findViewById<BottomNavigationView>(R.id.bottomNavigation)
                    bottomNavigation.selectedItemId =
                        bottomNavigation.menu.getItem(context.PROFILES_TAB).itemId
                }
    ...
    private fun setupProductRec(view: View): RecyclerView {
        ...
        prodRec.layoutManager =
            GridLayoutManager(this.context, 1, GridLayoutManager.HORIZONTAL, false)
        prodRec.addItemDecoration(
            DividerItemDecoration(
                this.context,
                DividerItemDecoration.VERTICAL
            )
    ...
    private fun setupTransactionsRec(view: View): RecyclerView {
        ...
        val onDeleteClicked = fun(trans: Transaction, person: Person) {
            ...
            val snackbarText = if (trans.product == null) {
                "Eenmalige inkoop \"${trans.messageOrProductName}\" van ${trans.getPerson(
                    realm,
                    trans.personId
                ).name} verwijderd"
            } else {
                "${trans.amount.toFormattedAmount()} ${trans.messageOrProductName} van ${trans.getPerson(
                    realm,
                    trans.personId
                ).name} verwijderd"
            }
        ...
        val amountRec = view.findViewById<RecyclerView>(R.id.mainAmountRec)
        transActionRec.adapter = TransactionRecAdapter(
            this.context!!,
            transactions,
            amountRec,
            realm,
        ...
        transActionRec.addItemDecoration(
            DividerItemDecoration(
                this.context,
                DividerItemDecoration.VERTICAL
            )
    ...
    private fun setupSpacingForTurfRec(columns: Int) {
        ...
        spacer = ConsistentSpacingDecoration(
            16.toPixel(this.context!!),
            16.toPixel(this.context!!),
            columns
        )
    ...
    override fun onSelectionChanged(selecting: Boolean) {
        ...
        rec.setPadding(
            ...
            rec.paddingTop,
            rec.paddingRight,
            newBotPad.toPixel(this.context!!)
        )
